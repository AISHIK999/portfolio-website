<template>
  ...
  >
    <Motion
      tag="div"
      class="relative [transform-style:preserve-3d] w-full h-full flex items-center justify-center"
      :animate="{
    ...
    >
      <div v-if="$slots.default" class="w-full h-full [transform:translateZ(0)]">
        <slot />
      </div>
      <img
        v-else-if="imageSrc"
      ...
      <Motion
        v-if="displayOverlayContent && (overlayContent || $slots.overlay)"
        tag="div"
